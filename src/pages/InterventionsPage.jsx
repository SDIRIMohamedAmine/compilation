...
export default function InterventionsPage() {
  ...
  const handleAdvance = async (id) => {
    ...
    try {
      const r = await advanceIntervention(id);
      setMsg({ ok: true, text: `${r.statut_avant} → ${r.statut_apres}` });
      await refetch();
      if (selected?.intervention_id === id) {
        const updated = data?.find(i => i.intervention_id === id);
        if (updated) setSelected({ ...updated, statut: r.statut_apres });
      }
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <select value={filter} onChange={e => setFilter(e.target.value)}>
              ...
              <option value="tech1_assigne">tech1_assigne</option>
              <option value="tech2_valide">tech2_valide</option>
              <option value="ia_valide">ia_valide</option>
          ...
          <div className="card" style={{ padding: 0, overflow: "hidden" }}>
            ...
            {data && (
              <table className="data-table">
                <thead><tr><th>ID</th><th>Capteur</th><th>Zone</th><th>Statut FSM</th><th>Priorité</th><th>Date</th><th></th></tr></thead>
                <tbody>
                  {data.map(i => (
                    <tr key={i.intervention_id} style={{ cursor: "pointer", background: selected?.intervention_id === i.intervention_id ? "var(--bg-3)" : undefined }}>
                      ...
                      <td>
                        <button className="btn" style={{ fontSize: 10, padding: "3px 8px" }} onClick={() => setSelected(i)}>
                          →
                        </button>
                      </td>
                    </tr>
          ...
          <div style={{ width: 320, flexShrink: 0 }}>
            <div className="card fade-up">
              ...
              <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 6, fontSize: 12 }}>
                {[["Capteur", `C-${selected.capteur_id}`],["Zone", selected.zone_nom],
                  ["Priorité", selected.priorite],["Tech 1", selected.tech1_nom || "—"],
                  ["Tech 2", selected.tech2_nom || "—"],["IA validée", selected.validation_ia ? "oui" : "non"]
                ].map(([k, v]) => (
              ...
              {selected.statut !== "termine" && (
                <button
                  className="btn btn-primary" style={{ marginTop: 14, width: "100%", justifyContent: "center" }}
