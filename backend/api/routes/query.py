...
"""
api/routes/query.py
NL -> SQL endpoint with full pipeline trace + ambiguity fallback.
"""
import sys, os
...
from compiler.ambiguity       import suggest_alternatives
...
def nl_query(body: dict):
    ...
    trace = {
        ...
        "error":       None,
        "error_phase": None,
        "suggestions": [],   # populated on failure
        "ambiguous":   False,
    }
    ...
    try:
        ...
        trace["error"] = str(e)
        trace["error_phase"] = "LEXER"
        trace["suggestions"] = suggest_alternatives(text)
        trace["ambiguous"]   = True
        return trace

    if not tokens:
        trace["error"] = "Aucun token reconnu"
        trace["error_phase"] = "LEXER"
        trace["suggestions"] = suggest_alternatives(text)
        trace["ambiguous"]   = True
        return trace
    ...
    try:
        ...
        trace["ast"] = {
            ...
            "order":       {"column": ast.order.column, "direction": ast.order.direction}
                           if ast.order else None,
            "aggregation": {"func": ast.aggregation.func, "column": ast.aggregation.column}
                           if ast.aggregation else None,
        }
    except ParseError as e:
        trace["error"] = str(e)
        trace["error_phase"] = "PARSER"
        trace["suggestions"] = suggest_alternatives(text)
        trace["ambiguous"]   = True
        return trace
...
def _fmt_condition(cond):
    ...
    if isinstance(cond, ConditionNode):
        left  = f"{cond.left.table}.{cond.left.column}" \
                if isinstance(cond.left, AttributeNode) else str(cond.left)
        right = cond.right.value \
                if isinstance(cond.right, ValueNode) else str(cond.right)
...
def query_examples():
    return [
        ...
        {"nl": "Affiche les capteurs signales",                 "hint": "filter statut"},
        {"nl": "Quelles interventions sont urgentes ?",         "hint": "filter priorite"},
    ]
